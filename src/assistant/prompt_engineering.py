...
OUTPUT_FORMAT_REQUIREMENT = """Structure your response logically as a business report. Use clear headings for each section. Where comparisons are made (e.g., competitors), use Markdown tables for clarity. Conclude with a dedicated 'Actionable Recommendations' section, suggesting concrete next steps, potential KPIs to track, and considerations for implementation within a mid-size company context."""
...
def get_detailed_competitive_analysis_prompt(
    ...
    prompt += "4.  **Comparative Analysis:** Create a Markdown table comparing OurCompany and the key competitors across the specified `focus_areas`. Use the search context and general knowledge. Analyze factors like:\n"
